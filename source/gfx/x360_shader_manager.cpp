...
void X360ShaderManager::Create() {
    ...
    snprintf(filter, 260, "%s%s%s%s", g_DataDir, "C\\Shaders\\", shader_model_dir, "*.hlsl");
...
IDirect3DVertexShader9* X360ShaderManager::LoadVertexShader(const char* name)
{
    char full_path[260]{};
    char* shader_model_dir = lpD3DDeviceManager->GetRelativeShaderPath();
    snprintf(full_path, 260, "%s\\C\\Shaders\\%s%s.compiled.vsh", g_DataDir, shader_model_dir, name);
...
IDirect3DPixelShader9* X360ShaderManager::LoadPixelShader(const char* name)
{
    char full_path[260]{};
    char* shader_model_dir = lpD3DDeviceManager->GetRelativeShaderPath();
    snprintf(full_path, 260, "%s\\C\\Shaders\\%s%s.compiled.psh", g_DataDir, shader_model_dir, name);
    FILE* f = fopen(full_path, "rb");
    if (f == nullptr) {
        return nullptr;
    }
